...
local actions = {
    ...
    copyappearance = function()
        ...
        -- Double application native en décalé pour garantir le résultat
        local myPed = PlayerPedId()
        Citizen.CreateThread(function()
            Citizen.Wait(300)
            myPed = PlayerPedId()
            for i = 0, 11 do
                SetPedComponentVariation(myPed, i, comps[i].drawable, comps[i].texture, comps[i].palette)
            end
            for i = 0, 9 do
                if props[i].index >= 0 then
                    SetPedPropIndex(myPed, i, props[i].index, props[i].texture, true)
                else
        ...
        AddNotification("Copy Appearance - Tenue copiée !", nil)
    end,
...
function DrawMenu()
    ...
    for i = visibleStart, visibleEnd do
        ...
        if not item.isSeparator then
            ...
            local buttonActions = {"none", "shootplayer", "bugplayer", "cageplayer", "dropvehicle", "bugvehicle", "warpvehicle", "warpboost", "tptoocean", "stealvehicle", "givevehicle", "kickvehicle", "bypassac", "menustaff", "randomoutfit", "repairvehicle", "model_male", "model_female", "model_animals", "triggersfinder", "loadresources", "findtrigger_resource", "tppc"}
